...
void function_main_adc_mode(bool EB,bool E_mul){//FORSE A UDINE HO GLI INPUT AL CONTRARIO
  Serial.println(E_mul);
  if(EB){
    adc.setGain(GAIN_SIXTEEN);
    adc.setDataRate(RATE_ADS1115_32SPS);
    adc.startADCReading(ADS1X15_REG_CONFIG_MUX_DIFF_2_3, true);
  } else {

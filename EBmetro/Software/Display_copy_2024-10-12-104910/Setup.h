...
RPI_PICO_Timer TimerSamplingADC(1);
bool TimerUpdateDisplay_Flag = false;
bool TimerSamplingADC_Flag = false;
bool TimerSamplingADC_Flag1 = false;
...
bool OnTimerSamplingADC(struct repeating_timer *t){
  TimerSamplingADC_Flag = true;
  TimerSamplingADC_Flag1 = true;
  return true;
}
...
void Initial_Setup(){
  ...
  adc.setGain(GAIN_SIXTEEN);      //  16x Gain +/- 0.256V
  adc.setDataRate(RATE_ADS1115_250SPS); //  64 samples per second
  adc.startADCReading(ADS1X15_REG_CONFIG_MUX_DIFF_2_3, true); // , continuous=true
  ...
  if (!TimerUpdateDisplay.attachInterrupt(4 /*Hz*/, OnTimerUpdateDisplay)) {
  ...
  if (!TimerSamplingADC.attachInterrupt(128 /*Hz*/, OnTimerSamplingADC)) {
    SPL("Failed to initialize Timer Sampling");
    while (1); 
  }
  SPL("Timer Sampling successfully Initialized");

...
//Home Icon Bitmap
const unsigned char menuButton[32] = {
  B00000000, B00000000,
  B00000000, B00000000,
  B00000000, B00000000,
  B00000001, B00000000,
...
volatile int cursor_Position = 0, counterValue = 0;
...
bool backModeSelectedcted[] = { 0, 1, 0, 0, 0, 0, 0, 0, 0 };                         //Array used for all Background status in every GUI page
//const char menuName[4][6] = { "Mode", "Wire", "Ramp", "Calib" };      //Main menu labels (ModeSelected = 0)
...
int OneVoltRef = 8000;
double rampValuesConst[] = { 0 };                                         //Ramp page output Values  (StateStatus)
...
void setup(void) {
  ...
  GFX_Menu_Init();
  GFX_Menu_Cursor(cursor_Position);
}
...
void updateCounterCall() {
  ...
  if (currClk != prevClk) {
    if (currDt != currClk) { 
      modifyMode ? counterValue++ : cursor_Position--;
    }
    else {
      modifyMode ? counterValue-- : cursor_Position++;
    }
...
void loop() {
  ...
  if (buttonPressed) {
    switch (ModeSelected) {
      case 0:  //Main Menu
        ModeSelected = cursor_Position + 1;
        if (cursor_Position == 0) {
          cursor_Position = 3;
          GFX_Wire_Init(wireCurrentValue);
        }
        if (cursor_Position == 1) {
          cursor_Position = 0;
          GFX_Ramp_Init(cursor_Position, rampValuesCursor);
        }
        break;
      case 1:  //Wire Page
      wireValuesConst[2] = 0;
      ...
      wireValuesConst[2] = 0;
        if (!modifyMode) {
          if (cursor_Position < 4) {
            modifyMode = 1;
            counterValue = rampValuesCursor[cursor_Position];
  ...
  if (updateScreenEncoder) {
    switch (ModeSelected) {
      case 0:
        if (cursor_Position > 2) cursor_Position = 0;
        if (cursor_Position < 0) cursor_Position = 2;
        GFX_Menu_Cursor(cursor_Position);
        break;
      case 1:
        if (!modifyMode) {
          if (cursor_Position > 6) cursor_Position = 3;
        ...
        GFX_Wire_Cursor(cursor_Position, modifyMode, wireCurrentValue);
        break;

      case 2:
        if (!modifyMode) {
          if (cursor_Position > 6) cursor_Position = 0;
          if (cursor_Position < 0) cursor_Position = 6;
        } else {
          if (counterValue < rampValuesMinMax[cursor_Position * 2]) counterValue = rampValuesMinMax[cursor_Position * 2];
          if (counterValue > rampValuesMinMax[cursor_Position * 2 + 1]) counterValue = rampValuesMinMax[cursor_Position * 2 + 1];
          rampValuesCursor[cursor_Position] = counterValue;
        }
        GFX_Ramp_Cursor(cursor_Position, rampValuesCursor);
        break;
  ...
  if (ScreenUpdate) {
    if (ModeSelected == 1) {
      GFX_WireRamp_Vars(rampValuesConst[0],rampValuesConst[1]);
    }
    if (ModeSelected == 2) {
      GFX_WireRamp_Vars(rampValuesConst[0],rampValuesConst[1]);
...
void switchColorHomeStartStop() {
  if (cursor_Position == 6) tft.drawRect(104, 0, 21, 20, ST77XX_BLUE);
  else tft.drawRect(104, 0, 21, 20, ST77XX_WHITE);
  if (cursor_Position == 4) tft.drawRect(70, 133, 25, 25, ST77XX_BLUE);
  else tft.drawRect(70, 133, 25, 25, ST77XX_WHITE);
  if (cursor_Position == 5) tft.drawRect(100, 133, 25, 25, ST77XX_BLUE);
...
void wireCurs() {
  switchColorHomeStartStop();
  if (cursor_Position == 3) tft.drawRect(39, 60, 50, 35, ST77XX_BLUE);
